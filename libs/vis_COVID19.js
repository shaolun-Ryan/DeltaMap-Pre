...
let colorHandle = (num)=>{
    if(num>=10000000){
        return 'red';
    }else if(num<10000000&&5000000<num){
        return 'pink';
    }else if(num<=5000000&&1000000<=num){
        return 'grey';
    }else if(num<2000000){
        return 'black';
    }
...
let partyHandle = (item)=>{
    return item==='Republican Party'?'red':'blue';
}

let addiAttr = (data,add,item,i)=>{
    // console.log(data,add,item,i)
    if (!add) {
        return d3.schemeCategory20[i%20]
    }
    if(add === 'mask-order'){
        return partyHandle(item)
    }
...
let vis_COVID19 = (svg, data, c=[], r=[], add=false,display='inc')=>{
    /*
    ...
    */
    ...
    let links = svg.append('g')
        ...
        .attr('stroke', (d,i) => {
            let color;
            color = addiAttr(data, add,data.link[i].Party,i)
            // console.log(color)
    ...
    function mouseover(d,i){
        ...
        select('#tooltip')
            ...
            .style('left', (event.offsetX+450) + "px")
